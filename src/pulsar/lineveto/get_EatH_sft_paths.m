function [sfts, firstsft] = get_EatH_sft_paths ( params_init, sftstartfreq, num_sfts_to_load, IFO )
 ...
 for numsft = 1:1:num_sfts_to_load
  ...
  if ( exist(sftfile,"file") != 2 )
   freqsubdir = sprintf("%04.0f", 10*floor((sftstartfreq + (numsft-1)*params_init.sft_width)/10+SMALL_EPS)); # EatH SFTs on atlas are organized in subdirs like "0050" - SMALL_EPS is to make sure 60.000 gets floored to 60 and not 50, as octave can have small numerical inaccuracies here
