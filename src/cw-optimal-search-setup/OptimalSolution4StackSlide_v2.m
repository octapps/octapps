...
function [sol, funs] = OptimalSolution4StackSlide_v2 ( varargin )
  ...
  if ( isempty ( sol ) )
    ...
    for i = 1:length(trial)
      ...
      trial{i}.sol = iterateSolver ( trial{i}.solverFun, trial{i}.startGuess, funs, tol, maxiter );
      if ( !isempty ( trial{i}.sol ) && !i_constraints_violated ( trial{i}.sol, i_constraints, tol ) )
